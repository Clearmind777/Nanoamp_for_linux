...
annotation_cache_dir <- function() {
  ...
  base <- if (nzchar(xdg)) xdg else file.path(path.expand("~"), ".cache")
  file.path(base, "nanoamp", "ref")
}
...
.http_get_once <- function(url, params = list(), dest = NULL, timeout = 60L) {
  ...
  if (!nzchar(curl_bin)) {
    stop("curl is required for annotation but was not found on PATH", call. = FALSE)
  }
...
.ensembl_json <- function(path, cache_key = NULL) {
  cache <- if (is.null(cache_key)) NULL else
