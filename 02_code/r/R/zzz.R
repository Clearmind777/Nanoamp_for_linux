...
# and score() out of Biostrings into the pwalign package. Biostrings keeps
# working as long as pwalign is attached, but older nanoamp code called
# Biostrings::pairwiseAlignment() directly, which fails on current
# Bioconductor releases with:
#   'pairwiseAlignment' is not an exported object from 'namespace:Biostrings'
#
# Resolve the provider once at load time so the R-native alignment backend
