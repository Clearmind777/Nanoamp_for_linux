test_that("GUI server runs a Mode C analysis", {
  ...
  reads <- file.path(repo_root, "01_data", "ln_test_data", "TSM20260826", "E4-3", "reads.fastq")
  ref <- file.path(repo_root, "01_data", "ln_test_data", "TSM20260826", "E4-3", "reference.self.fa")
  skip_if_not(file.exists(reads) && file.exists(ref), "ln_test_data not available")
