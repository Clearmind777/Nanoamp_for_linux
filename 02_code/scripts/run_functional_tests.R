#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# 功能测试：三种模式 × self/wt 参考
# 输入按 01_data/manifest.tsv 的“逻辑名 -> 真实 test_data 文件”映射解析
# 结果写入 04_results/r/test_run_local/
# ---------------------------------------------------------------------------
...
if (!file.exists(manifest_path)) {
  stop("请先运行 02_code/scripts/prepare_test_data.R 生成 01_data/manifest.tsv",
       call. = FALSE)
}
...
get_role <- function(dataset_name, sample_name, role_name, cluster_num = NULL) {
  ...
  # manifest 的 path 相对 01_data/
  file.path(data_root, q$path[1])
}
