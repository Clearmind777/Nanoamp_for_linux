...
`linux-x86_64/nanoamp-r-runtime.tar.gz.part*` 是一个可移植的 R 4.4.3 运行时，
包含所有必需的 R 包和 `nanoamp` 包：

- 核心 R 包：`data.table`、`optparse`、`jsonlite`、`readxl`、`Matrix`；
- Bioconductor：`Biostrings`、`Rsamtools`、`ShortRead`、`IRanges`、
  `GenomicAlignments`、`DECIPHER`；
- GUI：`shiny`、`DT`；
- `minimap2` 和 `samtools`。

运行时被切成 7 个 <100MB 的分片。第一次运行
`linux-x86_64/nanoamp` 或 `linux-x86_64/nanoamp-gui` 时，启动器会自动重组分片、
校验 SHA256、解压运行时并执行 `conda-unpack`。
...
在具有网络的 Linux x86_64 机器上重新构建运行时：
